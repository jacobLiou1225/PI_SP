...
func numStringToString(a int, b rune) string {
	simbe := b
	num := a
	result := " "
	result = fmt.Sprintf("%c%d", simbe, num)
	return result
}
...
func main() {
	f, err := excelize.OpenFile("DOC1_PI.SP.PO.xlsx")
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	for i, n := range readPiContent.Body.Pi.PiItems {
		ItemNumIdex, _ := CoordinatesToCellName(1, 19+i)
		GradeIdex, _ := CoordinatesToCellName(2, 19+i)
		EdgeIdex, _ := CoordinatesToCellName(3, 19+i)
		SizeIdex, _ := CoordinatesToCellName(4, 19+i)
		QuantityIdex, _ := CoordinatesToCellName(6, 19+i)
		UnitPriceIdex, _ := CoordinatesToCellName(7, 19+i)
		AmountIdex, _ := CoordinatesToCellName(10, 19+i)
	...
	if err := f.SaveAs("sssNew_DOC1_PI.SP.PO.xlsx"); err != nil {
		fmt.Println(err)
	}
